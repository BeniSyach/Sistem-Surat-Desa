<x-app-layout>
    <div class="container mx-auto px-4 py-6">
        <div class="flex items-center justify-between mb-6">
            <h1 class="text-2xl font-bold">Persetujuan Surat Keluar (Sekdes)</h1>
        </div>
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
            <div class="p-4 border-b">
                <form action="{{ route('outgoing-letters.sekdes-approval') }}" method="GET" class="flex flex-wrap gap-4">
                        ...
                        @forelse($letters as $letter)
                            <tr>
                                ...
                                <td>
                                    <span @class([
                                        'badge',
                                        'badge-warning' => $letter->status === 'pending_sekdes',
                                        'badge-success' => $letter->status === 'approved_sekdes',
                                        'badge-error' => $letter->status === 'rejected_sekdes',
                                    ])>
                                        @if ($letter->status === 'pending_sekdes')
                                            Menunggu Persetujuan
                                        @elseif($letter->status === 'approved_sekdes')
                                            Disetujui
                                        @elseif($letter->status === 'rejected_sekdes')
                                            Ditolak
                                        @endif
                                    </span>
                                </td>
                                <td>
                                    <div class="flex gap-2">
                                        ...
                                        @if ($letter->status === 'pending_sekdes')
                                            <form action="{{ route('outgoing-letters.sekdes-approve', $letter) }}"
                                            ...
                                            <form action="{{ route('outgoing-letters.sekdes-reject', $letter) }}"
                                                method="POST" class="inline">
                                                @csrf
